        ...
        {products.map(product => (
          <li key={product.produktiID}>
            ...
            <div className="d-grid gap-2 d-md-block w-100">
              <button className="btn btn-primary w-100" type="button" onClick={() => addToCart(product)}>Add To Cart</button>
              {/* {userRole === 'Admin' && ( // Render the Edit button only for Admin users
                <Link to={`/edit/${product.produktiID}`} className="btn btn-secondary w-100 mt-2">Edit</Link>
              )} */}
            </div>
          </li>
        ))}
